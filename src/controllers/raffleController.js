import { Users, WLRaffles, RafflesInfo, Winners, History } from "../models";
...
export const saveHash = async (req, res, next) => {
  const data = req.body.data;
  const historyResult = await History.find({ hash: data.hash });
...
export const create = async (req, res, next) => {
  ...
  if (isNFT) {
    const raffle = await WLRaffles.find({ raffleId });
    if (raffle.length === 0) {
...
export const checkNFT = async (req, res, next) => {
  ...
  const raffle = await WLRaffles.findOne({ nftId });
  if (raffle) {
...
//     const rr = await WLRaffles.findOneAndUpdate(
//       { _id: raffleId },
...
export const updateWLwinners = async (_id, users) => {
  ...
  for (let i = 0; i < users.length; i++) {
    let userInfo = await RafflesInfo.findOne({
      raffleId: _id,
      userid: users[i],
    });
...
export const getWinner = async (req, res, next) => {
  ...
  const winners = await Winners.find({ raffleId: raffleId });
